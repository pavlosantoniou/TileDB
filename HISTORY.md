...
## Improvements
* This change adds a new Array API:  which allows for the assignment of a config directly to an array. The config also has 2 new parameters for a start and end timestamp. This is to allow for a new change in the API . Within this PR, all  APIs have been deprecated and the  API has been changed to allow for a timestamp taken from the config. The next step will be changing that API further to take 2 timestamps, allowing for a open_between rather than an open_at. [#2142](https://github.com/TileDB-Inc/TileDB/pull/2142)
